...
| Max-entropy method (stretch #6) | DONE (synthetic-validated) | ✅ 6 tests | `methods/max_entropy.py`: MaxEnt deconvolution (multiplicative updates, flat prior, reduced-χ² stopping; Vasquez 1981 DOI 10.1016/0368-2048(81)85037-2, Aspnes 2022 DOI 10.3390/e24091238). **Kernel FWHM is a REQUIRED user input** (no default — the calibration claim is the user's, no invention); noise σ user-suppliable else MAD-estimated (flagged). HONESTY: `peaks=[]` (sharpening, not quantification); AMPLIFIES-NOISE warning verbatim in payload; artifact bound pinned relative to real features (~9% on synthetic). Deterministic. **With this, the FULL decision-matrix method menu (1–6) is implemented.** Codex checkpoint pending. |
