...
def test_real_ucl4_bn_n1s_main_peak_still_found_and_still_sharp():
    ...
    with an essentially unchanged, narrow width estimate -- this change
    must not distort characterization of features that were already
    handled correctly."""
    from vgd_parser import parse_vgd
...
def test_negative_control_flat_slope_sigmoid_no_meaningful_fp_rate_increase():
    ...
    flat/sloped/sigmoid, effectively-featureless region? Measured
    empirically before implementing (30 draws/condition, real N1s file's
    own ROI size [181 pts, step 0.1] and baseline count level [~6000]):
    old fixed ladder vs the new derived ladder gave statistically
    indistinguishable false-positive rates at the frozen CWT_PROM_Z_MIN
    gate (1.67% vs 1.67% in one measured run). This test re-derives that
    same aggregate check as a regression guard: the observed FP rate
    across many draws must stay in the same ballpark as the pre-existing
    baseline, not jump by an order of magnitude. A single-draw hard
    zero-tolerance assertion would be dishonest -- the true baseline FP
    rate at this operating point is measured, not zero."""
    n, step = 181, 0.1
    x = np.arange(0.0, n * step, step)[:n] + 380.0
    n_draws = 40
    n_fp = 0
    for seed in range(n_draws):
        for level, kind in ((6000.0, "flat"), (6000.0, "slope"), (6000.0, "sigmoid")):
            rng = np.random.default_rng(900_000 + seed * 3
            ...
            if any(f.prom_z >= CWT_PROM_Z_MIN for f in feats):
                n_fp += 1
    fp_rate = n_fp / (n_draws * 3)
    assert fp_rate <= 0.15, (
        f"false-positive rate at the frozen gate jumped to {fp_rate:.2%} "
        "on featureless data -- the wider ceiling is letting the detector "
        "latch onto background curvature (measured pre-fix baseline was "
        "~1.7-4.4% across several ROI sizes; this bound gives generous "
        "headroom above that, not zero-tolerance, since some baseline "
        "false-positive rate is expected and already tolerated by design)"
    )
